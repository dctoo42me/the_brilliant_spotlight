...
window.Modal = (() => {
    ...
    // 🧭 NEW: helper to manage URL updates cleanly
    function updateUrl(businessId, versionParam) {
    ...
    function open(business) {
        ...
        try {
            // ✅ Part 1 — build clean base URL
            const baseUrl = `${window.location.origin}${window.location.pathname}`;
            const timestamp = Date.now();
            const shareUrl = `${baseUrl}?business=${encodeURIComponent(business.id)}&v=${timestamp}`;
            ...
            updateUrl(business.id, timestamp); // 🧭 new helper replaces direct pushState
    ...
    function close(silent = false) {
        if (!isSharing) {
            ...
            if (!silent) updateUrl(null); // 🧭 new: only clears URL if not silent
        }
    }
    ...
    window.addEventListener("popstate", () => {
        ...
        if (businessId) {
            ...
            close(true); // 🧭 silent close on back navigation
        }
    });
